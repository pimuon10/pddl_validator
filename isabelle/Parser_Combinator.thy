...
definition "exactly ts \<equiv> foldr (\<lambda>t p. do { x\<leftarrow>get; if x=t then p \<bind> return o op # x else error id}) ts (return []) 
...
fun repeat :: "('t,'a) parser \<Rightarrow> ('t,'a list) parser" where
  "repeat p ::= optional [] (ensure_cparser p --[op#] repeat p)"

abbreviation "repeat1 p \<equiv> p --[op#] repeat p"
